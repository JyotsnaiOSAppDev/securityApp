...
final class GCMModeWorker: BlockModeWorker, FinalizingEncryptModeWorker, FinalizingDecryptModeWorker {
  ...
  func encrypt(block plaintext: ArraySlice<UInt8>) -> Array<UInt8> {
    ...
    gf.ghashUpdate(block: ciphertext)
     // let stringFromByteArray = String(data: Data(bytes: plaintext), encoding: .utf8)
     // print("encrpty")
  ...
  func decrypt(block ciphertext: ArraySlice<UInt8>) -> Array<UInt8> {
    ...
    // ciphertext block ^ ek1
      ...
      let arrData : Data = Data(plaintext)
      if ( arrData != nil ) {
          let encryptedDataText = arrData.base64EncodedString(options: NSData.Base64EncodingOptions())
         // NSLog("Encrypted with pubkey: %@", encryptedDataText)
      }

      if  let string = String(data: arrData, encoding: .utf8) {
         // print("array data to string ")
...
//      let bytes = plaintext.toHexString()
//      let base64Encoded   = plaintext.toBase64()
//      var decodedString = ""
//      if let decodedData = Data(base64Encoded: base64Encoded) {
//          print(decodedData)
//          let str = String(decoding: decodedData, as: UTF8.self)
//          print(str)
//
//      }
